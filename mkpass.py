...
def hash_password(password, salt=None, iterations=100000, algorithm="sha256", use_pepper=False):
    """
    ...
    Args:
        ...
        use_pepper: Boolean indicating whether to use a pepper with the password.
...
def main():
    ...
    parser.add_argument("--pepper", action="store_true", help="Use a pepper when hashing the password")

    args = parser.parse_args()

    # Pass the pepper option to the hash_password function
    hashed_password, salt = hash_password(args.password, args.salt, args.iterations, args.algorithm, use_pepper=args.pepper)
